...
<form action="activationcode" class="forms-sample" name="form1" method="post" style="transform:translate(80px)" > 
<table >
<tr>
<td align="center" colspan="2">
<label for="nbvouchers">Lookup : <b>Activation Code</b></label>
</td>
<td>
<input type="text" class="form-control" id="nbvouchers" pattern="[0-9]*" style="width:263px;height:40px;font-size:15px" maxlength="14" name="lookup" value="<?php if(isset($_POST['submit'])){ echo $_REQUEST["lookup"];} else { echo  "";}?>" required>
</td>
</tr>

<tr align="center" colspan="2">
<td align="center" colspan="2">
<button type="submit" name="submit" class="btn btn-gradient-primary me-2" style="transform:translate(265px,10px)">Envoyer</button>
<a href="http://10.7.24.2/minsat/dashboards/activationcode"><button type="submit"  class="btn btn-light" style="transform:translate(265px,10px)">Clear</button></a>
</td>
</tr>
...
$role=$_SESSION['role'];
$ac=$_POST["lookup"];
$sqlss="insert into audit (`login`,`role`,`datesn`,`ip`,`os`,`browser`,`activationcode`) 
values ('".$user."','".$role."','".$datee."','".$ip."','".$os."','".$browser."','".$ac."')  ";
$resultss=mysqli_query($connecta,$sqlss);	
...
<value><string>".$_POST["lookup"]."</string></value>
</member>
</struct>
</value>
</param>
...
// }
        ...
        $url = "10.13.0.49:10020/VoucherAdmin";
        $page = "/Air";
        $headers = array(
...
substr(SUBSTRING_INDEX(champ, 'i4', -1),4,1) as state , substr(SUBSTRING_INDEX(champ, 'subscriberId', -1),22,8)as subscriber2,
...
<br/>		
 <?php
while($row = mysqli_fetch_array($resultxa)){
	if(substr($row["batchid"],1,5)== "value"){
?>
<br/>
<br/>
<center><label><b>The Voucher does not exist</b></label></center>

<?php
}else{
?>
<center>
<table id="a" class="display nowrap" cellspacing="0" width="50%" style="border-width:1px;border-color: black;border-style: solid;transform:translate(-75px)">
<thead>
<tr style="background:#ff6600; color:#fff;font-weight: bold">
<th style="border-width:1px;border-color: black;border-style: dotted">Attribute</th>
<th style="border-width:1px;border-color: black;border-style: dotted">Value</th>
 </tr>
</thead>
...
<td style="border-width:1px;border-color: black;border-style: dotted"><b>BatchId</b></td>
...
<tr style="background:#fff;color:#000">  						 
<td style="border-width:1px;border-color: black;border-style: dotted"><b>Currency</b></td>
<td style="border-width:1px;border-color: black;border-style: dotted">TND</td>
</tr>
<tr style="background:#fff;color:#000">  						 
<td style="border-width:1px;border-color: black;border-style: dotted"><b>ExpiryDate</b></td>
<td style="border-width:1px;border-color: black;border-style: dotted">
<?php
echo $row["expiry"];
...
<tr style="background:#fff;color:#000">  						 
<td style="border-width:1px;border-color: black;border-style: dotted"><b>State</b></td>
<td style="border-width:1px;border-color: black;border-style: dotted">
<?php
$queryxaxx = "SELECT substr(SUBSTRING_INDEX(champ, 'i4', -1),4,1) as state FROM `lookup` where champ like '%VoucherExpi%' order by id desc limit 1 ";
$resultxaxx = mysqli_query($connect,$queryxaxx);

if (mysqli_num_rows($resultxaxx) != 0)
{
while($rowx = mysqli_fetch_array($resultxaxx)){	
if($rowx["state"]=="0")
{echo "AVAILABLE (EXPIRED)";}
elseif($rowx["state"]=="1")
{echo "USED : ".$row["used"] ;}
elseif($rowx["state"]=="5")
{echo "UNAVAILABLE";}
...
<?php if($row["state"]=="1") {?>
<tr style="background:#fff;color:#000">  						 
<td style="border-width:1px;border-color: black;border-style: dotted"><b>subscriberId</b></td>
<td style="border-width:1px;border-color: black;border-style: dotted">
<?php
$subs=$row["subscriber2"];
// echo substr($subs,0,5);
// echo "XXX";
echo $subs;
?>
</td>
...
<tr style="background:#fff;color:#000">  						 
<td style="border-width:1px;border-color: black;border-style: dotted"><b>Value</b></td>
<td style="border-width:1px;border-color: black;border-style: dotted">
<?php
echo $row["valeur"];
...
<tr style="background:#fff;color:#000">  						 
<td style="border-width:1px;border-color: black;border-style: dotted"><b>VoucherGroup</b></td>
<td style="border-width:1px;border-color: black;border-style: dotted">
<?php
echo $row["vg"];
