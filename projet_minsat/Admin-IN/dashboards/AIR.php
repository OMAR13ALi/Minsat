...
<table id="a" class="display" style="width:100%">
        <thead>
            <tr style="background:#ff6600; color:#fff;font-weight: bold;border-style: dashed;height:30px">
                ...
                <th style="border-width:1px;border-color: black;border-style:dashed">TN1AIR3</th>
                <th style="border-width:1px;border-color: black;border-style:dashed">TN2AIR4</th>
            </tr>

        </thead>
        <tbody>
<?php
if (($csvfile= fopen("../../OTN_HealthChecks/AIR/".$date_dir."/".$iteration."/checksAIR.csv", "r")) !==FALSE)
{ 
...
<br/>
		...
		var memberA_table = document.getElementById("AIR_table");
		var filename = "../../OTN_HealthChecks/AIR/"+<?php print($date_dir); ?>+"/"+<?php print($iteration); ?>+"/"+"AIR_"+col+"_"+row;
		var myWindow = window.open(filename,"MsgWindow","width=800,height=600");
		}

		
		$(document).ready(function(){
			$('#AIR_table td.table_values').each(function(){
				if ($(this).text() == '[NOK]') {
