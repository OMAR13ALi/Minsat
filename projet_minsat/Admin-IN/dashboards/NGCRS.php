...
<table id="a" class="display" style="width:100%">
        <thead>
            <tr style="background:#ff6600; color:#fff;font-weight: bold;border-style: dashed;height:30px">
                ...
                <th style="border-width:1px;border-color: black;border-style:dashed">TN2NGCRS2h2</th>
            </tr>

        </thead>
        <tbody>
<?php
if (($csvfile= fopen("../../OTN_HealthChecks/NGCRS/".$date_dir."/".$iteration."/checksNGCRS.csv", "r")) !==FALSE)
{ 
...
<br/>
		...
		var memberA_table = document.getElementById("NGCRS_table");
		var filename = "../../OTN_HealthChecks/NGCRS/"+<?php print($date_dir); ?>+"/"+<?php print($iteration); ?>+"/"+"NGCRS_"+col+"_"+row;
		var myWindow = window.open(filename,"MsgWindow","width=800,height=600");
		}

		
		$(document).ready(function(){
			$('#NGCRS_table td.table_values').each(function(){
				if ($(this).text() == '[NOK]') {
