...
const Offer = () => {
 ...
 const [offerState, setOfferState] = useState({
  ...
  useEffect(() => {
    setOfferState(prev => ({ ...prev, filteredData: offerData }));
  }, []);
  ...
  const DataTable = ({ 
    ...
    const getUniqueValues = (column) => {
      return [...new Set(data.map(item => item[column]))].sort((a, b) => {
        if (column === 'id') return Number(a) - Number(b);
          ...
          <table style={tableStyle}>
            ...
            <tbody>
              {paginatedData.map((row, rowIndex) => (
                <tr key={row.id + '-' + rowIndex} style={{ backgroundColor: rowIndex % 2 === 0 ? '#f9fafb' : 'white' }}>
