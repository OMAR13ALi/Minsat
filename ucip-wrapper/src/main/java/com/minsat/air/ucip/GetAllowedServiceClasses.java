...
public class GetAllowedServiceClasses {
    ...
    public GetAllowedServiceClasses(AirConfig config, AirTransport airTransport, XmlParser xmlParser) {
        this.config = config;
    ...
    public AirResponse execute(String subscriberNumber) {
        ...
        String xml = XmlHelper.wrapMethodCall("GetAllowedServiceClasses", members);
        try {
            return xmlParser.parseResponse(airTransport.send(xml, "GetAllowedServiceClasses"), txnId);
