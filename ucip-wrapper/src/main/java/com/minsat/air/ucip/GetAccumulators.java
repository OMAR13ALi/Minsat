...
public class GetAccumulators {
    ...
    public GetAccumulators(AirConfig config, AirTransport airTransport, XmlParser xmlParser) {
        this.config = config;
    ...
    public AirResponse execute(String subscriberNumber) {
        ...
        String xml = XmlHelper.wrapMethodCall("GetAccumulators", members);
        try {
            return xmlParser.parseResponse(airTransport.send(xml, "GetAccumulators"), txnId);
