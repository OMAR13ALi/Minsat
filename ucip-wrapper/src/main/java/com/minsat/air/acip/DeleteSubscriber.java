...
public class DeleteSubscriber {
    ...
    public DeleteSubscriber(AirConfig config, AirTransport airTransport, XmlParser xmlParser) {
        this.config = config;
     ...
     * @param originOperatorId  optional operator ID
     */
    public AirResponse execute(String subscriberNumber, String originOperatorId) {
        ...
        String xml = XmlHelper.wrapMethodCall("DeleteSubscriber", members.toString());
        try {
            return xmlParser.parseResponse(airTransport.send(xml, "DeleteSubscriber"), txnId);
